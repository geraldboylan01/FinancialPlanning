...
doc.addImage(
  ...
  if (latestRun.inputs.rentalIncome) {
    summaryText += `, rental income of ` +
                   `${fmtEuro(latestRun.inputs.rentalIncome)} per year`;
  }

  if (latestRun.inputs.hasDb && latestRun.inputs.dbPension) {
    summaryText += `, and a defined-benefit pension of ` +
                   `${fmtEuro(latestRun.inputs.dbPension)} ` +
